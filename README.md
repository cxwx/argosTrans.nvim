<!-- ********************************************************************** -->
<!--                                                                        -->
<!--                                                     :::      ::::::::  -->
<!-- README.md                                         :+:      :+:    :+:  -->
<!--                                                 +:+ +:+         +:+    -->
<!-- By: chenxu <6235641+cxwx@users.noreply.githu  +#+  +:+       +#+       -->
<!--                                             +#+#+#+#+#+   +#+          -->
<!-- Created: 2025/08/22 18:24:09 by chenxu           #+#    #+#            -->
<!-- Updated: 2025/08/22 18:26:59 by chenxu          ###   ########.fr      -->
<!--                                                                        -->
<!-- ********************************************************************** -->

<!-- cspell:ignore orcid -->

# Translate plugin using argos-translate

A NeoVim plugin to translate using argos 

## Installation

With [lazy.nvim](https://github.com/folke/lazy.nvim):

```lua
{
    "cxwx/argosTrans.nvim",
    opts = {},
    keys = {
        { ",t1", "<cmd>TransArgosInsert<CR>", mode = "v", desc = "insert" },
        { ",t2", "<cmd>TransArgosShow<CR>", mode = "v", desc = "show" },
    }
},
```

## Requirement

* `argos-translate`

```
pip install argostranslate
```


## Feature

* Trans English to Chinese


## Commands

The plugin provides the user command `:TransArgosInsert`, `:TransArgosShow`.
Invoke it when the cursor is on the name of a repo.

## TODO

- [ ] other engine(ollama?)
- [ ] language configure
